# 🐍 Cobra Faminta 2.0

O clássico jogo da cobrinha, com frutas especiais que dão poderes.

**Jogue aqui:** https://michaelquezada.github.io/cobra-faminta/

## Como jogar

- **Setas** ou **WASD** para mover
- **Espaço** para pausar
- No celular, deslize o dedo

## Frutas

| Fruta | Efeito |
|---|---|
| 🔴 Maçã | +1 ponto |
| 🟡 Dourada | pontos em dobro por 8s |
| 🔵 Gelo | tempo mais lento por 6s |
| 🟣 Fantasma | atravessa paredes por 7s |

A cobra acelera conforme você pontua. O recorde fica salvo no navegador.

## Rodar localmente

É um único arquivo HTML: basta abrir `index.html` no navegador.
